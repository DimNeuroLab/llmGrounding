...
Settlers_description_null=""
Settlers_description_base = "Settlers of Catan is a board game, this is a description: Settlers of Catan is a competitive game in which players throw 2 dice, collect resources, trade resources and try to win. Each player possesses structures adjacent to some land, and the sum of the thrown dice tell which lands will produce resources for that turn (for the players who own structures adjacent to them). There are only 5 kinds of resources: ore, wood, clay, wheat and sheep. If the sum of the rolled dice is 7, the active player moves the robber pawn to a new land and steals resources from other players who have built structures adjacent to that tile. Structures cost different combinations of resources, thus players need to trade with each other to get the resources they need."
Settlers_description=Settlers_description_null
def get_api_key():
...
def build_prompt(input_sample):
    ...
    prompt = f"""{Settlers_description}
    I will give you a dialogue from a game of Settlers, you will need to continue it with 1 possible utterance from 1 player, and 1 class.
    ...
    Very important: please respond with 1 possible continuation in this precise format: [type of utterance], [name of the player]:[utterance]
    """
...
def write_results_to_file(y_true, y_pred, y_true_parsed, y_pred_parsed):
    ...
    with open('n_turns_' + n_turns + '_n_context_' + n_context +'_' + ts + '.tsv', 'w') as out_file:
...
if __name__ == '__main__':
    ...
    for idx, sample in enumerate(tqdm(x_data)):
        ...
        if divmod(idx,10)=0:
            print("evaluations at "+idx+" samples")
