...
def build_prompt(input_sample):
    ...
    And this is an example of a dialogue with the associated class of utterance in brackets:
    ...
    player_blue: anythign else is fine (Offer)
    ...
    It is very important that you consider what was said by each player, which represent their intentions, and the order in which each player spoke. Build (but don't write) the framework of which resources each player wants to trade for giving and which to trade for receiving.
