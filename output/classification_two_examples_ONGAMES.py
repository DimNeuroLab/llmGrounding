...
def build_prompt_to_classify(input_sample, sample_to_classify):
    ...
    if len(input_sample)>0:
        for context in input_sample:
            ...
            dialogue += playersmap[context[0]] + ': ' + str(context[1]) + '\n'
...
if __name__ == '__main__':
    ...
    y_pred_raw = []
    y_true_raw = []
    for idx, sample_to_classify in enumerate(tqdm(y_data[1:])):
        input_sample = x_data[idx]  # Get the corresponding input_sample
        if len(input_sample) > 0:
            prompt = build_prompt_to_classify(input_sample, sample_to_classify)
            ...
            y_true_all.append(sample_to_classify[-1].lower())
            y_true_raw.append('; '.join(sample_to_classify))
