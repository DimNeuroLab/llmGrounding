...
def build_prompt_to_classify(input_sample, sample_to_classify):
    ...
    # Define the probabilities for each dialogue act given the previous act
    probabilities_text = """
    Previous Act: 'Offer'
    - Offer: ≈ 14.97%
    - Refusal: ≈ 53.45%
    ...
    # Split probabilities into blocks
    blocks = probabilities_text.strip().split("\n\n")
    
    # Shuffle the order of blocks
    random.shuffle(blocks)
    
    # Reconstruct probabilities text with shuffled blocks
    shuffled_probabilities_text = "\n\n".join(blocks)
    
    prompt = f"""I will give you a dialogue from a game of Settlers of Catan, you will need to decide the class of its last utterance.
    ...
    As a suggestion, consider the probability that the continuation (the next utterance) is of a certain dialogue act, based on the dialogue act of the last utterance of the dialogue:
