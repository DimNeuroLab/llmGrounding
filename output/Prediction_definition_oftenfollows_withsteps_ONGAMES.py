...
def build_prompt(input_sample):
    ...
    These are the patterns of how 
    
    - If it is an Offer: half of the times the next utterance is a Refusal; otherwise, in order of likelihood, it is Accept, or Offer, or Counteroffer
    - If it is a Counteroffer: the next utterance is, in order of likelihood, an Accept, a Counteroffer, a Refusal or Offer.
    - If it is an Accept: the next utterance is, in order of likelihood, an Accept, an Offer, a Counteroffer or a Refusal
    - If it is a Refusal: most of the times the next utterance is a Refusal; otehrwise, in order of likelihood, it is an Offer, or a Counteroffer, or an Accept.

    In order to compute the task (of predicting the class of the next utterance), please follow these steps:

    1) Given your understanding of the dialogue class from the definitions and the examples, help yourself with the trends to recognize associate each utterance of the dialogue with a dialogue act (don't write this)
    2) Focus then on the last utterance of the dialogue. What class of utterance could follow?
